...
__device__ void init_invert_page_table(VirtualMemory *vm) {
  ...
  for (int i = 0; i < vm->PAGE_ENTRIES; i++) {
    ...
    vm->invert_page_table[i + vm->PAGE_ENTRIES * 2] = 0; // initiaize the LRU
  }
}
  ...
  /* Complate vm_read function to read single element from data buffer */
    ...
    for (int physical_frame_number = 0; physical_frame_number < vm->PAGE_ENTRIES; physical_frame_number++) {
      if (vm->invert_page_table[physical_frame_number] == 0x00000000) { // not an empty entry
        if (vm->invert_page_table[physical_frame_number + vm->PAGE_ENTRIES] == virtual_page_number) { // found in page table
          for(int i=0; i<vm->PAGE_ENTRIES; i++){
...
__device__ void vm_write(VirtualMemory *vm, u32 addr, uchar value) {
  /* Complete vm_write function to write value into data buffer */
    ...
    for (int physical_frame_number = 0; physical_frame_number < vm->PAGE_ENTRIES; physical_frame_number++) {
        if (vm->invert_page_table[physical_frame_number] == 0x00000000) { // not an empty entry
            if (vm->invert_page_table[physical_frame_number + vm->PAGE_ENTRIES] == virtual_page_number) { // found in page table
                ...
                vm->invert_page_table[physical_frame_number + vm->PAGE_ENTRIES * 2] = 0; // add LRU bit
    ...
    *(vm->pagefault_num_ptr) += 1; // a page fault
    // look for invalid(empty) entries
    for (int physical_frame_number = 0; physical_frame_number < vm->PAGE_ENTRIES; physical_frame_number++) {
